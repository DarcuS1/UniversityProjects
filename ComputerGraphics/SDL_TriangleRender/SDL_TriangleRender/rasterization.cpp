    ...
    void rasterizeTriangle(SDL_Renderer* renderer, const std::vector<egc::vec4>& triangleVertices, const std::vector<egc::vec4>& triangleColors) {
        ...
        for (int y = minY; y <= maxY; y++) {
            ...
            for (int x = minX; x <= maxX; x++) {
                ...
                computeAlphaBetaGamma(triangleVertices, egc::vec2(x, y), alpha, beta, gamma);
                if (alpha >= 0 && beta >= 0 && gamma >= 0) {
                    egc::vec4 color = triangleColors[0]* alpha +  triangleColors[1] * beta +  triangleColors[2]* gamma;
